...
function Assert-NoHostPathDependencies {
    ...
    foreach ($file in Get-ChildItem -LiteralPath $PSScriptRoot -File -Recurse) {
        ...
        # PetaLinux/Yocto build trees are disposable generated caches.  They
        # necessarily record the machine-local workspace used by BitBake and
        # are never consumed when recreating a project from project-spec.
        # Portability is enforced on maintained sources and packaged outputs,
        # not on those caches.
        if ($relative -match '(?i)\\petalinux\\build\\[^\\]+\\') {
            continue
        }
        # Vivado implementation reports are immutable build evidence and
        # include the workspace used for that particular implementation.
        # The portable Tcl/RTL/XDC sources and exported XSA are checked below.
        if ($relative -match '(?i)\\vivado\\artifacts\\.*\.(rpt|log|jou)$') {
            continue
        }
...
function Assert-SdBootCommand {
    ...
    $bootargs = if ($SideName -eq 'RX') {
        'setenv bootargs root=/dev/ram0 rw mem=384M'
    } else {
...
try {
    ...
    foreach ($relative in @(
        ...
        'Jetson_Dashboard/assets/index-4E97A4AB.js',
        ...
        'docs/2026-08-12_3-1_FINAL_JTAG_SD_validation.md'
    )) {
        Assert-File $relative 'PC RX 01 UI and current detector documentation'
    }
    Assert-TextContains `
        -RelativePath 'Jetson_Dashboard/assets/index-4E97A4AB.js' `
        -RequiredText @('INTEGRITY ATTACK', 'WEAK-KEY SEARCH', 'Ve.length&&tl', 'video_throughput_mbps', 'board_power_w') `
        -Label 'Jetson two-page attack UI keeps local metrics independent of RX telemetry'
    Assert-TextDoesNotContain `
        -RelativePath 'Jetson_Dashboard/assets/index-4E97A4AB.js' `
        -ForbiddenText @('NORMAL FLOW') `
        -Label 'Jetson UI excludes PC-owned NORMAL FLOW page'
    Assert-TextContains `
        -RelativePath 'Jetson_Dashboard/backend/server.py' `
        -RequiredText @('_capture_for_session', 'live TX stream did not enter session') `
        -Label 'Weak-key demo validates the captured wired stream without RX telemetry'
    Assert-TextDoesNotContain `
        -RelativePath 'Jetson_Dashboard/backend/server.py' `
        -ForbiddenText @('RX telemetry did not enter the weak session') `
        -Label 'Weak-key preparation excludes removed RX-to-Jetson telemetry'
    Assert-TextContains `
    ...
    Assert-TextContains `
        ...
        -RequiredText @('PC_TELEMETRY_UART', 'ZYBO_RX_V1', 'telemetry_crc32', 'source_role', 'transport\":\"uart', 'discover_video_sender', 'PCAM_TX_PEER_FILE', 'detector_tag_total', 'detector_timeout_total') `
        -Label 'RX detector PC CRC-framed UART telemetry fields'
    Assert-TextDoesNotContain `
        -RelativePath 'AES_GCM_RX/petalinux/project-spec/meta-user/recipes-apps/aes-gcm-rx/files/aes-gcm-rx.c' `
        -ForbiddenText @('PC_TELEMETRY_GROUP', 'PC_TELEMETRY_BROADCAST', 'PC_TELEMETRY_DISCOVERY_PORT', 'PC_RX_UI_SUBSCRIBE_V1') `
        -Label 'RX PC telemetry excludes Wi-Fi UDP transport'
    Assert-TextContains `
        -RelativePath 'PC_RX_UI/server.py' `
        -RequiredText @('ZYBO_RX_V1', 'decode_uart_frame', 'uart_candidates', 'list_ports.comports', 'source_role', 'PC_RX_UI_PORT', 'create_server', 'JETSON_DASHBOARD_URL') `
        -Label 'PC UI runtime RX UART role discovery and local web endpoint'
    Assert-TextContains `
        -RelativePath 'session_control/aes_session_agent.c' `
        -RequiredText @('SO_BINDTODEVICE', 'interface_broadcast', 'waiting for RX discovery', 'discover_receiver_by_subnet', 'RX discovery unicast fallback found', 'CREATE_SECURE_SESSION', 'Never reuse a peer learned from the wired video path', 'MESSAGE_COUNTER_FLOOR', 'TX accepted authenticated RX counter floor', 'RX rejected stale session counter') `
        -Label 'Session exchange is bound to the USB Wi-Fi interface'
    Assert-TextDoesNotContain `
        -RelativePath 'session_control/aes_session_agent.c' `
        -ForbiddenText @('/run/aes-gcm-tx-peer', '/run/aes-gcm-rx-peer') `
        -Label 'Session exchange never reuses wired video peer files'
    foreach ($sessionInit in @(
        'session_control/aes-session-tx.init',
        'session_control/aes-session-rx.init'
    )) {
        Assert-TextContains `
            -RelativePath $sessionInit `
            -RequiredText @('AES_SESSION_DATA_DIR', '/var/lib/aes-session', 'automount path') `
            -Label "$sessionInit uses boot-mode-independent session state"
        Assert-TextDoesNotContain `
            -RelativePath $sessionInit `
            -ForbiddenText @('SD_ROOTFS_MOUNT', 'rootfs-mmcblk0p2', 'root-mmcblk0p2') `
            -Label "$sessionInit excludes fixed SD automount paths"
    }
    Assert-TextContains `
    ...
    foreach ($config in $configs) {
        ...
        if ($name -eq 'TX') {
            ...
            Assert-TextContains `
                -RelativePath "$root/vivado/rtl/aes256_gcm/video_aes_gcm_tx_top.sv" `
                -RequiredText @('session_key_valid && !crypto_ready') `
                -Label 'TX PL cipher reloads a level-held session key after runtime reset'
        }
