...
$work = Join-Path ([System.IO.Path]::GetTempPath()) 'pcam-aes-gcm\tx-frame-busy-sim'
...
$sources = @(
    ...
    (Join-Path $rtl 'video_aes_gcm_tx_top.sv'),
    (Join-Path $scriptDir 'tb_tx_frame_busy_boundary.sv')
)
...
try {
    ...
    & (Join-Path $vivado 'xelab.bat') tb_tx_frame_busy_boundary `
        -timescale 1ns/1ps -s tx_frame_busy_sim
    if ($LASTEXITCODE -ne 0) { throw 'xelab failed' }
    & (Join-Path $vivado 'xsim.bat') tx_frame_busy_sim -runall
